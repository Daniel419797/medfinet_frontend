...
export function BlockchainFeatureGate({
  feature,
  children,
}: {
    ...
    return (
      <section className="mx-auto max-w-2xl border border-amber-200 bg-amber-50 p-6 text-amber-950">
        ...
        <p className="mt-2 text-sm leading-6">
          Set <code className="font-bold">ALGORAND_ENABLED=true</code> on the backend and redeploy it. This page will appear automatically after the capability check refreshes.
        </p>
        {error && <p className="mt-3 text-sm font-semibold">{error}</p>}
        <button
          type="button"
          onClick={() => void refreshCapabilities()}
          className="mt-5 inline-flex items-center gap-2 border border-amber-300 bg-white px-4 py-2 text-sm font-bold"
        >
          <ArrowClockwise size={17} /> Recheck configuration
        </button>
      </section>
    );
  }
  ...
  if (!health?.reachable) {
    return (
      <section className="mx-auto max-w-2xl border border-rose-200 bg-rose-50 p-6 text-rose-950">
        ...
        <p className="mt-2 text-sm leading-6">Check the Algod server, network access, and backend wallet configuration before allowing transaction signing.</p>
        <button type="button" onClick={() => void refreshCapabilities()} className="mt-5 inline-flex items-center gap-2 border border-rose-300 bg-white px-4 py-2 text-sm font-bold">
          <ArrowClockwise size={17} /> Retry connection
        </button>
      </section>
    );
  }
...
export default BlockchainFeatureGate;
